export function createMoveTool(ctx) {
  ...
  return {
    ...
    activate() {
      ...
      previewPoint = null;
      clearOverlay();
      ctx.setStatus('Move: select entities, pick base point, pick target point');
    },
    deactivate() {
      basePoint = null;
      previewPoint = null;
    ...
    onPointerDown(event) {
      ...
      if (ctx.selection.entityIds.length === 0) {
        ...
        if (hit) {
          ...
          ctx.setStatus('Move: no selection');
          return;
        }
      }
      ...
      if (!basePoint) {
        ...
        ctx.setStatus('Move: pick target point');
        return;
      }
      ...
      const result = ctx.commandBus.execute('selection.move', { delta });
      ctx.setStatus(result.ok ? `Move applied dx=${delta.x.toFixed(2)} dy=${delta.y.toFixed(2)}` : (result.message || 'Move failed'));
      basePoint = null;
      previewPoint = null;
      clearOverlay();
    },
