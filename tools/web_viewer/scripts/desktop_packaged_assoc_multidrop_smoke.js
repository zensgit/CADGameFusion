...
const DEFAULT_OUTDIR = path.join(repoRoot, "build", "desktop_packaged_assoc_multidrop_smoke");
...
const EXPECTED_ASSOC_EXTENSIONS = ["dwg", "dxf", "cad"];
...
function usage() {
  return [
    "Usage: node tools/web_viewer/scripts/desktop_packaged_assoc_multidrop_smoke.js [--outdir <dir>] [--app <path>] [--input-dwg <path>] [--input-dwg-alt <path>] [--pack-if-needed]",
    "",
    "Launches the real packaged VemCAD app and verifies packaged file associations plus multi-file CAD drop queueing.",
  ].join("\n");
}
...
function readPackagedFileAssociations(appBinary) {
  const infoPlistPath = path.resolve(path.dirname(appBinary), "..", "Info.plist");
  ensure(fs.existsSync(infoPlistPath), `Info.plist not found: ${infoPlistPath}`);
  const result = spawnSync("plutil", ["-convert", "json", "-o", "-", infoPlistPath], {
    cwd: desktopDir,
    encoding: "utf8",
    stdio: "pipe",
  });
  if (result.status !== 0) {
    throw new Error(`plutil failed for ${infoPlistPath}:\n${result.stdout || ""}${result.stderr || ""}`);
  }
  const payload = JSON.parse(result.stdout || "{}");
  const documentTypes = Array.isArray(payload.CFBundleDocumentTypes) ? payload.CFBundleDocumentTypes : [];
  const declaredExtensions = documentTypes.flatMap((entry) =>
    Array.isArray(entry?.CFBundleTypeExtensions) ? entry.CFBundleTypeExtensions : []
...
async function readRecentUi(page) {
  return page.evaluate(() => ({
    entries: Array.from(document.querySelectorAll(".recent-file-item")).map((item) => ({
      ...
      lastOpenedAt: String(item.dataset.lastOpenedAt || ""),
      label: String(item.querySelector(".recent-file-btn")?.textContent || "").trim(),
      meta: String(item.querySelector(".recent-file-meta")?.textContent || "").trim(),
    })),
  }));
}
...
async function readBatchUi(page) {
  return page.evaluate(() => {
    const panel = document.querySelector("#desktop-batch-panel");
    return {
      visible: Boolean(panel) && !panel.classList.contains("is-hidden"),
...
async function run() {
  ...
  try {
    ...
    summary.unsupported_drop_path = path.join(outdir, "ignored-drop.txt");
    summary.router_url = routerUrl;
    summary.export_dir = exportDir;
    summary.smoke_home = smokeHome;
    summary.user_data_dir = userDataDir;

    fs.writeFileSync(summary.unsupported_drop_path, "not a CAD file\n", "utf8");

    summary.file_associations = readPackagedFileAssociations(appBinary);
    for (const ext of EXPECTED_ASSOC_EXTENSIONS) {
      ensure(summary.file_associations.declaredExtensions.includes(ext), `Expected packaged file associations to include .${ext}, got ${summary.file_associations.declaredExtensions.join(", ")}`);
    }
    ensure(!summary.file_associations.declaredExtensions.includes("json"), `Did not expect packaged file associations to claim .json, got ${summary.file_associations.declaredExtensions.join(", ")}`);
    ...
    await simulateCadMultiDrop(page, [inputDwg, summary.unsupported_drop_path, inputDwgAlt]);
    ...
    await page.waitForFunction((expectedPaths) => {
      ...
      if (items.length < expectedPaths.length) {
        return false;
      }
      return expectedPaths.every((expectedPath, index) => items[index]?.dataset.path === expectedPath);
    }, [inputDwgAlt, inputDwg], { timeout: 30000 });
    await page.waitForFunction(() => {
      const summary = String(document.querySelector("#desktop-batch-summary")?.textContent || "").trim();
      const items = Array.from(document.querySelectorAll(".desktop-batch-item"));
      return summary.includes("Complete")
        && items.some((item) => item.dataset.status === "ignored")
        && items.filter((item) => item.dataset.status === "opened").length >= 2;
    }, null, { timeout: 30000 });
    summary.after_multi_drop = {
      status: await readMainStatus(page),
      recent_ui: await readRecentUi(page),
      batch_ui: await readBatchUi(page),
    ...
    ensure(matchesOpenedLabel(summary.after_multi_drop.status, secondLabel), `Expected multi-drop final status for ${secondLabel}, got ${summary.after_multi_drop.status}`);
    ensure(summary.after_multi_drop.recent_ui.entries.length === 2, `Expected two recent UI entries after multi-drop, got ${JSON.stringify(summary.after_multi_drop.recent_ui.entries)}`);
    ensure(summary.after_multi_drop.recent_ui.entries[0]?.path === inputDwgAlt, `Expected top recent entry ${inputDwgAlt}, got ${JSON.stringify(summary.after_multi_drop.recent_ui.entries)}`);
    ensure(summary.after_multi_drop.recent_ui.entries[1]?.path === inputDwg, `Expected second recent entry ${inputDwg}, got ${JSON.stringify(summary.after_multi_drop.recent_ui.entries)}`);
    ensure(summary.after_multi_drop.recent_ui.entries.every((entry) => entry.meta.includes("Last opened:")), `Expected recent UI metadata to include timestamps, got ${JSON.stringify(summary.after_multi_drop.recent_ui.entries)}`);
    ensure(summary.after_multi_drop.batch_ui.visible, `Expected batch UI to be visible, got ${JSON.stringify(summary.after_multi_drop.batch_ui)}`);
    ensure(summary.after_multi_drop.batch_ui.summary === "Complete · 2 opened · 1 ignored", `Expected batch summary to report opened/ignored counts, got ${summary.after_multi_drop.batch_ui.summary}`);
    ensure(summary.after_multi_drop.batch_ui.items.some((item) => item.path === summary.unsupported_drop_path && item.status === "ignored"), `Expected ignored unsupported file item, got ${JSON.stringify(summary.after_multi_drop.batch_ui.items)}`);
    ensure(summary.after_multi_drop.batch_ui.items.filter((item) => item.status === "opened").length === 2, `Expected two opened batch items, got ${JSON.stringify(summary.after_multi_drop.batch_ui.items)}`);
    ensure((summary.after_multi_drop.bridge_recent.entries || []).length === 2, `Expected two bridge recent entries after multi-drop, got ${JSON.stringify(summary.after_multi_drop.bridge_recent)}`);
    ensure(findRecentMenuLabels(summary.after_multi_drop.menu_state).slice(0, 2).join("|") === [secondLabel, firstLabel].join("|"), `Expected recent menu order ${secondLabel}, ${firstLabel}; got ${JSON.stringify(findRecentMenuLabels(summary.after_multi_drop.menu_state))}`);
