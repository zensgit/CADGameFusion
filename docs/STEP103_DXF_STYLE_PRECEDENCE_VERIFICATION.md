...
## Scope
- Validate line style + color precedence for BYBLOCK, BYLAYER, and explicit entity styles.
...
- BYBLOCK + INSERT style wins over layer styles:
  - `LayerBlock` -> line type `CENTER`, line weight `0.5`, color `0xFF0000`.
  - `LayerNestedInsert` -> line type `DASHED`, line weight `0.2`, color `0x00FF00`.
- BYBLOCK with missing INSERT style falls back to layer styles:
  - `LayerByblockNoInsert` -> line type `DASHDOT`, line weight `0.7`, color `0xFFFF00`.
- BYLAYER resolves to layer styles (INSERT style ignored):
  - `LayerBylayer` -> line type `CENTER2`, line weight `0.25`, color `0x00FFFF`.
- LWPOLYLINE BYBLOCK uses INSERT style:
  - `LayerPolyByblock` -> line type `PHANTOM`, line weight `0.9`, color `0xFF00FF`.
- ARC BYLAYER uses layer styles:
  - `LayerArcBylayer` -> line type `PHANTOM2`, line weight `0.6`, color `0xC0C0C0`.
- Explicit entity styles override both INSERT and layer:
  - `LayerExplicit` -> line type `HIDDEN`, line weight `0.8`, color `0x0000FF`.
