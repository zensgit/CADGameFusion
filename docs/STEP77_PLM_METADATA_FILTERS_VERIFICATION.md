# Step 77: PLM Metadata Filters - Verification
...
## Manual smoke
```bash
cat > /tmp/plm_history_filters.jsonl <<'JSONL'
...
python3 tools/plm_router_service.py --history-file /tmp/plm_history_filters.jsonl --history-load 10 --port 10006
curl -s "http://localhost:10006/history?owner=alex&tags=review"
curl -s "http://localhost:10006/projects?owner=alex&tags=review"
python3 - <<'PY'
import base64
raw = "demo\nsample".encode("utf-8")
print(base64.urlsafe_b64encode(raw).decode("ascii").rstrip("="))
PY
curl -s "http://localhost:10006/documents/<document_id>/versions?revision=rev%20B"
```

## Results
- `py_compile`: PASS
- Manual smoke: PASS (filtered history/projects/versions responses)
