# Step333 Property Panel Render Raw Deps Transport Verification

## Scope

To verify Step333 after implementation, validate:
...
- `mkdir -p /tmp/editor-current-layer-step333 /tmp/editor-selection-summary-step333 && /opt/homebrew/bin/node /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_current_layer_smoke.js --outdir /tmp/editor-current-layer-step333 && /opt/homebrew/bin/node /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_selection_summary_smoke.js --outdir /tmp/editor-selection-summary-step333`
- `PATH="/Users/huazhou/.npm-global/bin:/opt/homebrew/bin:/usr/local/bin:$PATH" bash /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_ui_flow_smoke.sh --mode observe --outdir /tmp/editor-ui-flow-step333`
...
- pipeline still reads `resolvedDeps.rawDeps` instead of the explicit parameter
- entrypoint still mutates the resolved deps object
- rawDeps default changed from `{}` to something else
- step expands into pipeline order or branch behavior changes
