# Step338 Selection Overview Extraction Verification

## Scope

To verify Step338 after implementation, validate:
...
- unchanged presenter integration behavior through existing tests
...
- `mkdir -p /tmp/editor-current-layer-step338 /tmp/editor-selection-summary-step338 && /opt/homebrew/bin/node /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_current_layer_smoke.js --outdir /tmp/editor-current-layer-step338 && /opt/homebrew/bin/node /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_selection_summary_smoke.js --outdir /tmp/editor-selection-summary-step338`
- `PATH="/Users/huazhou/.npm-global/bin:/opt/homebrew/bin:/usr/local/bin:$PATH" bash /Users/huazhou/Downloads/Github/VemCAD/deps/cadgamefusion/tools/web_viewer/scripts/editor_ui_flow_smoke.sh --mode observe --outdir /tmp/editor-ui-flow-step338`
...
- overview helper starts owning badges or detail-fact behavior
- `selection_presenter.js` stops exporting `formatSelectionSummary(...)` or `formatSelectionStatus(...)`
- single-selection status wording changes
- multi-selection status loses read-only count behavior
- `buildSelectionContract(...)` or `buildSelectionPresentation(...)` behavior changes even though only summary/status logic was supposed to move
