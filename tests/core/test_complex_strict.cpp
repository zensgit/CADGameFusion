...
#define EXPECT_GE(a, b) assert((a) >= (b)); if(false) std::cout
#define EXPECT_LE(a, b) assert((a) <= (b)); if(false) std::cout
...
TEST_FUNC(ComplexStrictTest, LShapedWithTwoHoles) {
    const bool hasEarcut = (cadgf_get_feature_flags() & CADGF_FEATURE_EARCUT) != 0u;
    ...
    int index_count = 0;
    int success = cadgf_triangulate_polygon_rings(
        points.data(),
        ring_counts.data(),
    ...
    if (hasEarcut) {
        ...
        if (success) {
            // If succeeded, do basic validation
...
TEST_FUNC(ComplexStrictTest, SimplePolygonFallback) {
    ...
    int index_count = 0;
    int success = cadgf_triangulate_polygon(points.data(), static_cast<int>(points.size()), nullptr, &index_count);
    std::vector<unsigned int> indices;
    if (success && index_count > 0) {
        indices.resize(static_cast<size_t>(index_count));
        success = cadgf_triangulate_polygon(points.data(), static_cast<int>(points.size()), indices.data(), &index_count);
        indices.resize(static_cast<size_t>(index_count));
    }
...
TEST_FUNC(ComplexStrictTest, EmptyInput) {
    ...
    EXPECT_EQ(index_count, 0) << "Empty input should not produce any indices";
...
int main(int argc, char **argv) {
    ...
    std::cout << "[INFO] Feature flags: EARCUT=" << ((feats & CADGF_FEATURE_EARCUT) ? "on" : "off")
              << " CLIPPER2=" << ((feats & CADGF_FEATURE_CLIPPER2) ? "on" : "off") << std::endl;

    std::cout << "[INFO] Running without GTest (using assert-based tests)" << std::endl;
    
    // Manually run tests
    ComplexStrictTest_LShapedWithTwoHoles();
    ComplexStrictTest_SimplePolygonFallback();
    ComplexStrictTest_EmptyInput();
