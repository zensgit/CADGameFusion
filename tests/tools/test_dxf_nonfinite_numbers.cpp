...
int main(int argc, char** argv) {
    ...
    // Non-finite coordinates must be rejected during parse; invalid TEXT entities should be skipped.
    assert(type_counts[CADGF_ENTITY_TYPE_TEXT] == 0);
    ...
    assert(meta_int_or(doc, "dxf.text.entities_emitted", -1) == 0);
    assert(meta_int_or(doc, "dxf.text.skipped_missing_xy", -1) == 2);
    assert(meta_int_or(doc, "dxf.text.nonfinite_values", -1) == 2);
