...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < entity_count; ++i) {
        ...
        std::string kind;
        if (!query_entity_meta_value(doc, id, "text_kind", &kind) || kind != "mleader") {
            continue;
        }
        ...
        std::vector<char> buf(static_cast<size_t>(required));
        int required2 = 0;
        assert(cadgf_document_get_text(doc, id, &pos, &height, &rotation, buf.data(),
                                       static_cast<int>(buf.size()), &required2));
        const std::string value = std::string(buf.data());
        assert(value == "MLEADER_STEP186\nSECOND_LINE");
        assert(height == 2.5);
        assert(pos.x == 12.0);
        assert(pos.y == 18.0);
        found_mleader += 1;
